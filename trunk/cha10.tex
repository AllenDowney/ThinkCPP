...
number (called an index).  An {\tt apstring} is similar to a vector,
...
The vector type that appears on the AP exam is called {\tt apvector}.
In order to use it, you have to include the header file {\tt
apvector.h}; again, the details of how to do that depend on your
...
what it is.  The function we are invoking is an {\tt apvector}
...
There is another constructor for {\tt apvector}s that takes
...
There is one more constructor for {\tt apvector}s, which is
called a copy constructor because it takes one {\tt apvector}
...
{\tt apvector}s because there is a better alternative:

\begin{verbatim}
  apvector<int> copy = count;
\end{verbatim}
%
The {\tt =} operator works on {\tt apvector}s in pretty much
...
{\tt apvector}.  One of them is very useful, though: {\tt length}.
...
\end{verbatim}
%
The return type is {\tt apvector<int>}, which means that
...
\end{verbatim}
%
Notice that it is legal to pass {\tt apvector}s by reference.
